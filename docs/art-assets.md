...
| asset-id | 用途 | サイズ目安 | ステータス | 受領日 | 備考 |
|---|---|---|---|---|---|
| sf-hero-swordsman | 主人公キャラ（横向き、剣を構えるポーズ） | 256×384、背景透過 | 組み込み済み | 2026-09-05 | GameScene でプレイヤースプライトに使用（36×54へ縮小。当たり判定18×32は従来通り。未読込時はGraphics描画へフォールバック） |
| sf-enemy-normal | 通常敵キャラ | 256×256、背景透過 | 組み込み済み | 2026-09-05 | GameScene の normal タイプ敵に使用 |
| sf-enemy-agile | 敏捷型敵キャラ | SVG、背景透過 | 組み込み済み | 2026-09-21 | 青緑・細身・双剣の専用シルエット。agileタイプに使用 |
| sf-enemy-tank | タンク型敵キャラ | SVG、背景透過 | 組み込み済み | 2026-09-21 | 紫鋼・大型・盾持ちの専用シルエット。tankタイプに使用 |
| sf-bg-forest | 背景の森ステージイラスト | 1920×600、不透過（横スクロール想定） | 組み込み済み | 2026-09-05 | GameScene の背景（scrollFactor 0.4 のパララックス）。未読込時は青空グラデーションへフォールバック |
...
| asset-id | 用途 | サイズ目安 | ステータス | 受領日 | 備考 |
|---|---|---|---|---|---|
| cm-mascot | タイトル画面のマスコットキャラ | 512×512、背景透過 | 組み込み済み | 2026-09-05 | GameScene のタイトル画面（タイトル文字の左隣、120px、上下ゆらぎ）とリザルト画面（スコア右隣、72px）に使用。未読込時はタイトル文字を従来の中央揃えに戻す |
| cm-turbo-badge | ターボモード突入時に表示するバッジ/エフェクト素材 | 256×256、背景透過 | 組み込み済み | 2026-09-05 | GameScene のターボ突入時にポップ表示（Back.easeOut→0.7秒後フェード）＋ターボ中はHUDのターボ文言左に36pxで常時表示。未読込時は従来の 🔥 テキスト演出のみ |
...
| asset-id | 用途 | サイズ目安 | ステータス | 受領日 | 備考 |
|---|---|---|---|---|---|
| fl-hero-fighter | 主人公キャラ（構えポーズ） | 384×512、背景透過 | 組み込み済み | 2026-09-05 | GameScene バトル画面の左側（x=180、高さ250px・3:4維持）。ヒット時の横揺れtweenは画像にも同様に適用。未読込時は従来の青い角丸長方形（Graphics） |
| fl-enemy-fighter | 対戦相手キャラ | 384×512、背景透過 | 組み込み済み | 2026-09-05 | GameScene バトル画面の右側（x=620、高さ250px）。画像が左向きに描かれているため flipX 不要。未読込時は従来の赤い角丸長方形（Graphics） |
| fl-gacha-char-ryuga | ガチャキャラ「竜牙」の立ち絵 | 384×512、背景透過 | 組み込み済み | 2026-09-05 | ガチャ画面左の立ち絵スロット（高さ280px）に SSR「竜牙」排出時のみポップ表示。それ以外・未読込時は「？」プレースホルダー。版権IPを避けたオリジナルデザイン |
| fl-bg-arena | バトル背景（闘技場） | 1600×900、不透過 | 組み込み済み | 2026-09-05 | バトル画面（上下に半透明の暗帯を敷いてHUD/ボタンを読みやすく）とタイトル画面（暗幕0.55＋パネル透過0.82）の背景。800×600 にカバー配置（左右ははみ出し）。未読込時は従来の単色背景 |
...
| asset-id | 用途 | サイズ目安 | ステータス | 受領日 | 備考 |
|---|---|---|---|---|---|
| kq-hero-warrior | 勇者キャラ（デフォルト外見） | 384×512、背景透過 | 組み込み済み | 2026-09-05 | タイトル画面（見出し下・高さ170px）と討伐画面（上部・高さ130px）に表示。画像が無い場合はGraphicsの簡易シルエットにフォールバック |
| kq-faction-icon-warrior | 戦士の派閥アイコン | 128×128、背景透過 | 組み込み済み | 2026-09-05 | 育成画面の派閥ラベル上（76px）・「力を貸す」ボタン左（32px）・最終結果の「最も応えた派閥」（64px） |
...
| asset-id | 用途 | サイズ目安 | ステータス | 受領日 | 備考 |
|---|---|---|---|---|---|
| st-general-hakuen | 武将「白炎」(SSR)の立ち絵 | 384×512、背景透過 | 組み込み済み | 2026-09-05 | ガチャ結果の立ち絵枠（180×240）と武将一覧の行ポートレートに使用。未所持時は暗いシルエット表示 |
| st-general-soujin | 武将「蒼刃」(SR)の立ち絵 | 384×512、背景透過 | 組み込み済み | 2026-09-05 | 同上 |
| st-bg-battlefield | タップ進撃の背景（中華風の街道） | 1200×800、不透過 | 組み込み済み | 2026-09-05 | 全画面共通の背景（進撃画面は暗幕0.2で見せ、タイトル0.55・その他0.6で文字を優先） |
...
- 組み込み済み: 全6作 27点（ポーション工房 4、剣戟の森 5、カラーマッチ 2、覇拳伝 4、カルマクエスト 9、三国ポチポチ 3）。2026-09-21 に剣戟の森のagile/tank専用敵アート2点を追加
...
## 2026-09-06 統合改修の素材方針

今回の実装は既存アセットのみ。遠征の小兵・足軽はコード描画の兵士として表示し、固有立ち絵の代用であることを設計上明示する。次段階の任意素材候補は以下。まだ生成・受領していない。

| asset-id | 用途 | サイズ目安 | ステータス | 依頼プロンプト案 |
|---|---|---|---|---|
| st-general-kohei | 遠征の商才役「小兵」 | 384×512・透過 | 設計のみ | 中華風の若い従軍商人、金茶の衣装、小さな荷袋、正面全身、既存の蒼刃と並べるカジュアルソシャゲ風、くっきり輪郭、足先まで収める |
| st-general-ashigaru | 遠征の守将役「足軽」 | 384×512・透過 | 設計のみ | 中華風の歩兵、青緑の鎧と盾、頼れる穏やかな表情、正面全身、既存の蒼刃と並べるカジュアルソシャゲ風、くっきり輪郭、足先まで収める |

攻撃・被弾差分、勇者衣装、工房設備の追加セットは、次段階の画面配置が決まってから個別台帳化する。

## 2026-09-06 ボスを主役にする改修

ユーザー指示に基づき、内蔵imagegenで専用素材を制作して組み込む。蒼刃は画風参考とし、ボスは大きく、一般兵は簡潔な造形にする。

|asset-id|用途|状態|
|---|---|---|
|st-boss-gatekeeper|関門を守る大型ボス・透過立ち絵|生成・組み込み済み|
|st-general-kohei|金茶色の軽装兵・小兵の立ち絵|生成・組み込み済み|
|st-general-ashigaru|青緑の盾兵・足軽の立ち絵|生成・組み込み済み|

3点とも `games/sangoku-tap/public/images/<asset-id>.png` に保存。1086×1448、RGBAの透明ピクセルを検査済み。内蔵imagegenを使用し、CLI/APIは不使用。初期の兵士案はチェッカーが画像に焼き込まれたため不採用。透明なボス画像を画風参照に再生成した最終版を採用し、画像加工での色抜きは行っていない。

一般敵兵は足軽の絵を小さく左右反転・赤みのある色調で表示。小兵・足軽の味方アイコンは原色を維持。ボスは全地域で共通1体の絵を利用し、地域固有の3体を制作したわけではない。画像読み込み失敗時はコード描画の代替表示を残す。

最終プロンプトと参照の対応は [制作記録](design/sangoku-character-art.md) を参照。過去欄の「設計のみ」「未制作」は初回時点の履歴で、本項が最新状態。
