...
| sf-enemy-normal | 通常敵キャラ | 256×256、背景透過 | 組み込み済み | 2026-09-05 | GameScene の normal タイプ敵に使用（40×40へ縮小。agile/tank はティント付きプレースホルダーのまま） |
...
| kq-bg-kingdom-portrait-v2 | 縦画面の王都・王国背景 | 948×1659、不透過 | 受領 | 2026-09-15 | 内蔵imagegenで生成。基準モックの骨組みを優先するため縦選択画面では未使用。肉付け段階の候補として保留 |
