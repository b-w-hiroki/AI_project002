...
/**
 * 「覇拳伝」共通のビジュアルテーマ。格闘ゲームらしい重厚な配色（濃い赤茶〜金）を採用し、
 * 他2作の明るい配色とは差別化する。角丸パネル・ボタンのGraphicsユーティリティは共通構成。
 */
export const THEME = {
  panelFill: 0x2a2018,
  panelBorder: 0xc9a24a,
  shadow: 0x000000,
  accent: 0xd94a3d,
  textPrimary: "#f5ead2",
  textMuted: "#b8a888",
...
export function drawPanel(
  ...
  // 上面だけをわずかに明るくし、カードではなく「装備されたゲームUI」に見せる。
  ...
  // 対角の短いアクセントで格闘ゲームのフレーム感を足す。
  g.lineStyle(2, THEME.accent, 0.78);
  g.lineBetween(x - w / 2 + 12, y - h / 2 + 1, x - w / 2 + 36, y - h / 2 + 1);
  g.lineBetween(x + w / 2 - 36, y + h / 2 - 1, x + w / 2 - 12, y + h / 2 - 1);
  return g;
}
...
export function drawSpeakerIcon(scene: Phaser.Scene, x: number, y: number, on: boolean, size = 16): Phaser.GameObjects.Graphics {
  ...
  const color = on ? THEME.textPrimary : "#665c4a";
